...
def process_cache_map(
    ...
    for i in range(0, n, 3):
        ...
        if not meta_file.endswith(".meta.json.gz"):
            parser.error(
                "Invalid --cache-map meta_file %s (triple[1] must be *.meta.json.gz)" % meta_file
            )
        if not data_file.endswith(".data.json.gz"):
            parser.error(
                "Invalid --cache-map data_file %s (triple[2] must be *.data.json.gz)" % data_file
            )
